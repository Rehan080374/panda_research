...
void ForceExampleController::starting(const ros::Time& /*time*/) {
  ...
  Eigen::Map<Eigen::Matrix <double, 6, 1>> force_ext_measured(robot_state.K_F_ext_hat_K.data());
  Eigen::Map<Eigen::Matrix <double, 6, 1>> force_zero(robot_state.cartesian_collision.data());
  Eigen::Map<Eigen::Matrix<double, 7, 1>> gravity(gravity_array.data());
  //std::cout<<" meaured force = ",force_ext_measured;
  Eigen::VectorXd force_initial(6);
  initial_force=force_ext_measured-force_zero;
...
void ForceExampleController::update(const ros::Time& /*time*/, const ros::Duration& period) {
  ...
  Eigen::Map<Eigen::Matrix<double, 7, 1>> gravity(gravity_array.data());
  //R_x_=target_k_r_;
  Eigen::VectorXd tau_d(7), desired_force_torque(6), tau_cmd(7), tau_ext(7),force_ext(6);
  desired_force_torque.setZero();
  
  desired_force_torque=force_ext_measured-initial_force;
  // desired_force_torque(0)=-desired_force_torque(0);//y rot
  // desired_force_torque(1)=-desired_force_torque(1);
  // desired_force_torque(2)=-desired_force_torque(2);//y
  // desired_force_torque(3)=-desired_force_torque(3);//x
  // desired_force_torque(4)=desired_force_torque(4);
  // desired_force_torque(5)=-desired_force_torque(5);

  desired_force_torque(0) = desired_force_torque(0)*desired_mass_ ;
  desired_force_torque(1) = desired_force_torque(1)*k_p_;// made changes here----------------------------*****8
  desired_force_torque(2) = desired_force_torque(2)*k_i_ ;
  desired_force_torque(3) = desired_force_torque(3)*desired_mass1_;
  desired_force_torque(4) = desired_force_torque(4)*k_p1_;
  desired_force_torque(5) = desired_force_torque(5)*k_i1_;
  ...
  tau_cmd = tau_d-tau_ext;
  //tau_cmd = tau_d-tau_ext_initial_ ;//- tau_ext ;
  ...
  desired_mass_ = -target_mass_;
  k_p_ = target_k_p_ ;
  k_i_ = target_k_i_  ;
  desired_mass1_ = -target_mass1_;
  k_p1_ = target_k_p1_ ;
  k_i1_ = target_k_i1_ ;
...
void ForceExampleController::desiredMassParamCallback(
    ...
    uint32_t /*level*/) {
  target_mass_ = config.x;
  target_k_p_ = config.y;
  target_k_i_ = config.z;
  target_mass1_ = config.rx;
  target_k_p1_ = config.ry;
  target_k_i1_ = config.rz;
  //target_k_r_ = config.rx;
